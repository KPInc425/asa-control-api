...
import configRoutes from "./routes/configs/index.js";
import enhancedAuthRoutes from "./routes/enhanced-auth/index.js";
import logsRoutes from "./routes/logs.js";
import environmentRoutes from "./routes/environment/index.js";
import nativeServerRoutes from "./routes/native-servers/index.js";
import saveFilesRoutes from "./routes/save-files.js";
import discordRoutes from "./routes/discord.js";
import autoShutdownRoutes from "./routes/auto-shutdown.js";
import autoUpdateRoutes from "./routes/auto-update/index.js";
...
fastify.addHook("onSend", async (request, reply, payload) => {
  if (request.url.includes("/debug-rcon")) {
    ...
    logger.info(
      `[RESPONSE] Payload preview: ${payload ? payload.substring(0, 200) : "null"}`,
    );
  }
});
...
// Ensure built-in game adapters are registered before routes
await gameRegistry.ensureBuiltins();
logger.info(`Game adapters loaded: ${gameRegistry.ids.join(", ")}`);
...
logger.info("Games routes registered");
...
const setupSocketIO = () => {
  ...
  io.use((socket, next) => {
    logger.info(
      `Socket.IO authentication attempt from ${socket.handshake.address}`,
    );
    ...
    import("./services/user-management.js")
      .then(({ default: userManagementService }) => {
        try {
          const result = userManagementService.verifyToken(token);
          if (result.success) {
            socket.user = result.user;
            logger.info(
              `Socket.IO authentication successful for user: ${result.user.username}`,
            );
            next();
          } else {
  ...
  io.on("connection", (socket) => {
    logger.info(
      `Socket.IO client connected: ${socket.id} (User: ${socket.user?.username || "unknown"})`,
    );
    ...
    socket.on("start-container-logs", async (data) => {
      ...
      logger.info(
        `Starting container log stream for: ${container} (Socket: ${socket.id})`,
      );
      try {
        const dockerService = await import("./services/docker.js");
        const containerObj =
          dockerService.default.docker.getContainer(container);
    ...
    socket.on("start-ark-logs", async (data) => {
      ...
      logger.info(
        `Starting ARK log stream for: ${serverName}/${logFileName} (Socket: ${socket.id})`,
      );
      try {
        const arkLogsService = await import("./services/ark-logs.js");

        // Check if log file exists
        const exists = await arkLogsService.default.logFileExists(
          serverName,
          logFileName,
        );
        if (!exists) {
        ...
        const logStream = await arkLogsService.default.createLogStream(
          serverName,
          logFileName,
          {
            tail: 100,
        ...
        logStream.on("error", (error) => {
          logger.error(
            `ARK log stream error for ${serverName}/${logFileName}:`,
            error,
          );
          socket.emit("ark-log-error", {
            error: error.message,
        ...
        logger.error(
          `Failed to get ARK logs for ${serverName}/${logFileName}:`,
          error,
        );
        socket.emit("ark-log-error", {
          error: error.message,
...
const setupAutoUpdateSocketEvents = (io, updateService) => {
  ...
  updateService.on("auto-update:checking", (data) => {
    logger.debug(
      `[AutoUpdate] Broadcasting checking event for ${data.serverName}`,
    );
  ...
  updateService.on("auto-update:available", (data) => {
    logger.info(
      `[AutoUpdate] Broadcasting update available for ${data.serverName}`,
    );
  ...
  updateService.on("auto-update:warning", (data) => {
    logger.info(
      `[AutoUpdate] Broadcasting warning for ${data.serverName}: ${data.minutesRemaining}min remaining`,
    );
  ...
  updateService.on("auto-update:starting", (data) => {
    logger.info(
      `[AutoUpdate] Broadcasting update starting for ${data.serverName}`,
    );
  ...
  updateService.on("auto-update:progress", (data) => {
    logger.debug(
      `[AutoUpdate] Broadcasting progress for ${data.serverName}: ${data.message} (${data.percent}%)`,
    );
  ...
  updateService.on("auto-update:completed", (data) => {
    logger.info(
      `[AutoUpdate] Broadcasting update completed for ${data.serverName}`,
    );
  ...
  updateService.on("auto-update:failed", (data) => {
    logger.error(
      `[AutoUpdate] Broadcasting update failed for ${data.serverName}: ${data.error}`,
    );
  ...
  try {
    ...
    try {
      const { stateReconciliation } =
        await import("./services/state-reconciliation.js");
      stateReconciliation.destroy();
      logger.info("State reconciliation service destroyed");
    } catch (error) {
      logger.warn(
        "Error destroying state reconciliation service:",
        error.message,
      );
    }
...
const start = async () => {
  try {
    ...
    logger.info(
      "Socket.IO server created with CORS origins:",
      io.engine.opts.cors.origin,
    );
    ...
    logger.info(
      `ASA Control API server listening on ${config.server.host}:${config.server.port}`,
    );
    logger.info(
      `Socket.IO server ready on ${config.server.host}:${config.server.port}`,
    );
