...
export default async function installRoutes(fastify) {
  ...
  fastify.get(
    "/api/provisioning/wizard-options",
    {
    ...
    async (request, reply) => {
      try {
        ...
        return reply
          .status(500)
          .send({ success: false, message: "Failed to get wizard options" });
      }
    },
  );
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        return reply
          .status(500)
          .send({ success: false, message: "Failed to initialize system" });
      }
    },
  );
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        return reply
          .status(500)
          .send({ success: false, message: "Failed to install SteamCMD" });
      }
    },
  );
  ...
  fastify.post(
    "/api/provisioning/install-asa-binaries",
    {
      preHandler: requirePermission("write"),
    },
    async (request, reply) => {
      try {
        const { foreground = false, gameType = "ark" } = request.body;
        const result = await provisioner.installASABinaries(
          foreground,
          gameType,
        );
        return {
        ...
        return reply
          .status(500)
          .send({ success: false, message: "Failed to install ASA binaries" });
      }
    },
  );
  ...
  fastify.post(
    "/api/provisioning/update-server",
    {
      preHandler: requirePermission("write"),
    },
    async (request, reply) => {
      try {
        ...
        if (!serverName) {
          return reply
            .status(400)
            .send({ success: false, message: "Server name is required" });
        }
        ...
        return reply
          .status(500)
          .send({ success: false, message: "Failed to update server" });
      }
    },
  );
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        return reply
          .status(500)
          .send({ success: false, message: "Failed to update all servers" });
      }
    },
  );
}
