...
export default async function globalConfigRoutes(fastify) {
  ...
  fastify.get(
    "/api/provisioning/global-configs",
    {
      preHandler: requirePermission("read"),
    },
    async (request, reply) => {
      try {
        const gameType = request.query.gameType || "ark";
        ...
        try {
          gameUserSettingsIni = await fs.readFile(
        ...
        try {
          const globalSettingsData = await fs.readFile(
            globalSettingsPath,
            "utf8",
          );
        ...
        return reply.status(500).send({
          success: false,
          message: "Failed to get global configurations",
        });
      }
    },
  );
  ...
  fastify.put(
    ...
    async (request, reply) => {
      try {
        ...
        const adapter = gameFor(gameType || "ark");
        const effectiveGameType = gameType || "ark";
        const basePath =
          process.env.NATIVE_BASE_PATH || config.server.native.basePath;
        const globalConfigPath = path.join(
          basePath,
        ...
        return reply.status(500).send({
          success: false,
          message: "Failed to save global configurations",
        });
      }
    },
  );
  ...
  fastify.get(
    "/api/provisioning/config-exclusions",
    {
      preHandler: requirePermission("read"),
    },
    async (request, reply) => {
      try {
        ...
        return reply.status(500).send({
          success: false,
          message: "Failed to get configuration exclusions",
        });
      }
    },
  );
  ...
  fastify.put(
    ...
    async (request, reply) => {
      try {
        ...
        await fs.writeFile(
          exclusionsPath,
          JSON.stringify({ excludedServers }, null, 2),
          "utf8",
        );
        return {
          success: true,
        ...
        return reply.status(500).send({
          success: false,
          message: "Failed to save configuration exclusions",
        });
      }
    },
  );
