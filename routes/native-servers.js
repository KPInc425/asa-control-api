...
export default async function nativeServerRoutes(fastify, options) {
  // --- MOVE live-details route to the very top ---
  fastify.get(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        // Try server browser query via game adapter
        ...
        if (asaStats) {
          ...
          stateReconciliation.recordSuccessfulProbe(
            name,
            DataSource.QUERY,
            asaStats,
          );
        ...
        if (isRunning) {
          ...
          try {
            ...
            if (serverInfo && serverInfo.rconPort) {
              ...
              const playerList = await rconService.getPlayerList(
                name,
                rconOptions,
              );
              rconData = {
        ...
        const reconciledData = stateReconciliation.reconcileStatus(
          name,
          sources,
        );
        ...
        logger.info("[live-details] about to return (reconciled)", {
          ...
          details: {
            ...
            canonicalStatus: safeDetails.canonicalStatus,
            source: safeDetails.source,
          },
        });
        return { success: true, details: safeDetails };
      } catch (error) {
        logger.error("[live-details] error", {
          error: error.message,
          stack: error.stack,
        });
        ...
        const safeDetails = JSON.parse(JSON.stringify(details));
        logger.info("[live-details] about to return (error)", {
          success: true,
          details: safeDetails,
        });
    ...
    async (request, reply) => {
      try {
        ...
        if (
          ...
          (!config.rconPassword || config.rconPassword !== config.adminPassword)
        ) {
          config.rconPassword = config.adminPassword;
          logger.info(
            `Updated RCON password to match admin password for server ${name}`,
          );
        }
        ...
        fastify.log.error(
          `Error getting native server configuration for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
  ...
  fastify.delete(
    ...
    async (request, reply) => {
      try {
        ...
        fastify.log.error(
          `Error deleting native server configuration for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        fastify.log.error(
          `Error getting native server stats for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        const servers = await serverManager.listServers();
        const cluster = servers.find(
          (s) => s.type === "cluster" && s.name === name,
        );
        ...
        if (cluster) {
          ...
          serverManager.startCluster(name).catch((error) => {
            fastify.log.error(
              `Background cluster start failed for ${name}:`,
              error,
            );
          });
        ...
        fastify.log.error(
          `Error initiating native server start for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        fastify.log.error(
          `Error stopping native server ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        fastify.log.error(
          `Error restarting native server ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        fastify.log.error(
          `Error getting cluster server info for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        fastify.log.error(
          `Error getting start.bat for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        fastify.log.error(
          `Error listing log files for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
  ...
  fastify.put(
    ...
    async (request, reply) => {
      try {
        ...
        const result = await serverManager.updateClusterServerStartBat(
          name,
          content,
        );
        return result;
        ...
        fastify.log.error(
          `Error updating start.bat for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
      ...
      try {
        ...
        if (serverManager.regenerateServerStartScript) {
          ...
          return reply.status(400).send({
            success: false,
            message:
              "Start.bat regeneration is only available for native servers",
          });
        }
      } catch (error) {
        fastify.log.error(
          `Error regenerating start.bat for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        fastify.log.error(
          `Error checking running status for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        if (!server.rconPort) {
          logger.error(
            `RCON command failed: No RCON port configured for server ${name}`,
          );
          return reply.status(400).send({
            success: false,
        ...
        logger.info(
          `Sending RCON command to ${name} on port ${server.rconPort}: ${command}`,
        );
        ...
        const rconPassword =
          server.adminPassword || server.config?.adminPassword || "admin123";
        ...
        logger.info(
          `Sending RCON command to ${name} on ${rconHost}:${rconPort}: ${command}`,
        );
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        if (isRunning) {
          try {
            ...
            const { stdout } = await execAsync(
              `tasklist /FI "IMAGENAME eq ArkAscendedServer.exe" /FO CSV /NH`,
            );
        ...
        logger.error(
          `Error getting debug info for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        logger.info(
          `Step 1: Getting server configuration from database for ${name}`,
        );
        ...
        if (!dbServerConfig) {
          logger.warn(
            `Server ${name} not found in database, creating from cluster config`,
          );
          ...
          try {
            ...
            throw new Error(
              `Invalid server configuration in database for ${name}: ${parseError.message}`,
            );
          }
        }

        logger.info(
          `Step 2: Creating start script directly from database config for ${name}`,
        );

        // Ensure the admin password is set correctly
        const adminPassword =
          dbConfig?.adminPassword || serverConfig.adminPassword || "King5252";
        ...
        const updatedServerConfig = {
          ...
          customDynamicConfigUrl:
            dbConfig?.customDynamicConfigUrl ||
        ...
        if (server.isClusterServer && server.clusterName) {
          ...
          const clustersPath =
            ...
            join(process.env.NATIVE_BASE_PATH || "F:\\ARK", "clusters");
          serverPath = join(clustersPath, server.clusterName, name);
          logger.info(
            `Creating start script for cluster server ${name} at: ${serverPath}`,
          );
        } else {
          // Standalone server
          serverPath =
            serverConfig.serverPath ||
            join(process.env.NATIVE_BASE_PATH || "F:\\ARK", "servers", name);
          logger.info(
            `Creating start script for standalone server ${name} at: ${serverPath}`,
          );
        }

        try {
          const { ServerProvisioner } =
            await import("../services/server-provisioner.js");
          const provisioner = new ServerProvisioner();

          // Regenerate both config files and start script
          if (server.isClusterServer && server.clusterName) {
            // Regenerate config files in cluster
            await provisioner.createServerConfigInCluster(
              server.clusterName,
              serverPath,
              updatedServerConfig,
            );
            ...
            await provisioner.createServerConfig(
              serverPath,
              updatedServerConfig,
            );
            // Create start script for standalone server (without password args)
            await provisioner.createStartScript(
              serverPath,
              updatedServerConfig,
            );
          }
          ...
          logger.info(
            `Successfully regenerated config files and start script for ${name} with password: ${adminPassword.substring(0, 3)}***`,
          );
        } catch (createError) {
          logger.error(
            `Failed to regenerate config files and start script for ${name}: ${createError.message}`,
          );
          throw new Error(
            `Failed to regenerate config files and start script: ${createError.message}`,
          );
        }

        // Step 3: Log success and provide instructions
        logger.info(
          `Step 3: RCON fix completed for ${name}. Database updated with correct password.`,
        );
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        logger.error(
          `Error regenerating start script for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
        ...
        const debugInfo = {
          ...
          calculatedPaths: {
            ...
            serversPath:
              ...
              join(process.env.NATIVE_BASE_PATH || "F:\\ARK", "servers"),
          },
          clustersPath:
            process.env.NATIVE_CLUSTERS_PATH ||
            join(process.env.NATIVE_BASE_PATH || "F:\\ARK", "clusters"),
        ...
        for (const testPath of commonPaths) {
          try {
            ...
            if (exists) {
              ...
              const clustersExists = await fs
                .access(clustersSubPath)
                .then(() => true)
                .catch(() => false);
              debugInfo.commonPaths.push({
            ...
            for (const clusterDir of clusterDirs) {
              try {
                ...
                debugInfo.clusterConfigs[clusterDir] = {
                  exists: true,
                  serverCount: clusterConfig.servers
                    ? clusterConfig.servers.length
                    : 0,
                  serverNames: clusterConfig.servers
                    ? clusterConfig.servers.map((s) => s.name)
                    : [],
                  configPreview:
                    JSON.stringify(clusterConfig, null, 2).substring(0, 1000) +
                    "...",
                };
              } catch (error) {
                debugInfo.clusterConfigs[clusterDir] = {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      logger.info(
        `[DEBUG-TEST] Test endpoint called for ${request.params.name}`,
      );
      return {
        success: true,
        debug: {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        logger.info(
          `[DEBUG-RCON] Processing debug request for server: ${name}`,
        );

        // Get server info
        logger.info(`Debug RCON: Starting debug for server: ${name}`);
        logger.info(
          `Debug RCON: Server manager base path: ${serverManager.basePath}`,
        );
        logger.info(
          `Debug RCON: Environment NATIVE_BASE_PATH: ${process.env.NATIVE_BASE_PATH}`,
        );

        const servers = await serverManager.listServers();
        logger.info(
          `Debug RCON: Found ${servers.length} servers in list: ${servers.map((s) => s.name).join(", ")}`,
        );
        ...
        if (!server) {
          logger.warn(
            `Server ${name} not found in server list. Available servers: ${servers.map((s) => s.name).join(", ")}`,
          );
          return reply.status(404).send({
            success: false,
        ...
        logger.info(
          `Debug RCON: Found server ${name} with type ${server.type}, isClusterServer: ${server.isClusterServer}, clusterName: ${server.clusterName}`,
        );
        ...
        try {
          ...
          logger.warn(
            `Could not read start.bat for ${name}: ${fileError.message}`,
          );
        ...
        if (startBatContent) {
          const passwordMatch = startBatContent.match(
            /AdminPassword="([^"]+)"/,
          );
          if (passwordMatch) {
        ...
        try {
          ...
          try {
            ...
            gameUserSettingsContent = await fs.readFile(
              gameUserSettingsPath,
              "utf8",
            );
          } catch (error) {
            logger.warn(
              `Could not read GameUserSettings.ini for ${name}: ${error.message}`,
            );
          }
          ...
          try {
            ...
            logger.warn(
              `Could not read Game.ini for ${name}: ${error.message}`,
            );
          }
        } catch (configError) {
          logger.warn(
            `Could not access config directory for ${name}: ${configError.message}`,
          );
        }
        ...
        if (gameUserSettingsContent) {
          const rconEnabledMatch = gameUserSettingsContent.match(
            /RCONEnabled\s*=\s*(True|False)/i,
          );
          const rconPortMatch =
            gameUserSettingsContent.match(/RCONPort\s*=\s*(\d+)/);
          const adminPasswordMatch = gameUserSettingsContent.match(
            /ServerAdminPassword\s*=\s*([^\r\n]+)/,
          );

          rconEnabled = rconEnabledMatch ? rconEnabledMatch[1] : null;
          rconPort = rconPortMatch ? rconPortMatch[1] : null;
          configAdminPassword = adminPasswordMatch
            ? adminPasswordMatch[1].trim()
            : null;
        }
        ...
        const debugInfo = {
          ...
          environment: {
            NATIVE_BASE_PATH: process.env.NATIVE_BASE_PATH
              ? process.env.NATIVE_BASE_PATH.replace(/\\\\/g, "\\")
              : null,
            NATIVE_CLUSTERS_PATH: process.env.NATIVE_CLUSTERS_PATH
              ? process.env.NATIVE_CLUSTERS_PATH.replace(/\\\\/g, "\\")
              : null,
            NATIVE_SERVERS_PATH: process.env.NATIVE_SERVERS_PATH
              ? process.env.NATIVE_SERVERS_PATH.replace(/\\\\/g, "\\")
              : null,
            SERVER_MODE: process.env.SERVER_MODE,
          },
          serverInfo: {
          ...
          startBatInfo: {
            ...
            contentPreview: startBatContent
              ? startBatContent.substring(0, 500) + "..."
              : null,
          },
          passwordComparison: {
            ...
            allMatch:
              (server?.adminPassword === dbConfig?.adminPassword &&
                dbConfig?.adminPassword === configAdminPassword) ||
              false,
          },
          configFiles: {
            configsPath: configsPath
              ? configsPath.replace(/\\\\/g, "\\")
              : null,
            ...
            gameUserSettingsContent: gameUserSettingsContent
              ? gameUserSettingsContent.substring(0, 1000) + "..."
              : null,
            gameIniContent: gameIniContent
              ? gameIniContent.substring(0, 500) + "..."
              : null,
          },
        };
        ...
        logger.info(
          `Debug RCON: Return object keys:`,
          Object.keys(returnObject),
        );
        logger.info(
          `Debug RCON: Debug object keys:`,
          Object.keys(returnObject.debug),
        );
        ...
        try {
          ...
          logger.info(
            `Debug RCON: JSON string preview: ${jsonString.substring(0, 500)}`,
          );

          // Test parsing back
          const parsed = JSON.parse(jsonString);
          logger.info(
            `Debug RCON: Parsed back successfully, debug keys: ${Object.keys(parsed.debug || {}).join(", ")}`,
          );

          // Log individual debug object properties
          logger.info(`Debug RCON: serverName = ${parsed.debug?.serverName}`);
          logger.info(
            `Debug RCON: serverInfo keys = ${Object.keys(parsed.debug?.serverInfo || {}).join(", ")}`,
          );
          logger.info(
            `Debug RCON: adminPassword = ${parsed.debug?.serverInfo?.adminPassword}`,
          );
          logger.info(
            `Debug RCON: rconPort = ${parsed.debug?.serverInfo?.rconPort}`,
          );
        } catch (error) {
          logger.error(
            `Debug RCON: JSON serialization error: ${error.message}`,
          );
        }
        ...
        logger.error(
          `[DEBUG-RCON] Error debugging RCON for ${request.params.name}:`,
          error,
        );
      ...
      try {
        ...
        try {
          ...
          logger.info(`Testing RCON connection for ${name}:`, {
            ...
            port: rconPort,
            passwordLength: rconPassword ? rconPassword.length : 0,
            passwordPreview: rconPassword
              ? rconPassword.substring(0, 3) + "***"
              : "none",
          });
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        if (
          rawStatus.status === "starting" ||
          rawStatus.status === "stopping"
        ) {
          liveData.transition = {
            status: rawStatus.status,
            previousStatus: rawStatus.previousStatus,
            transitionStartedAt: rawStatus.startTime
              ? new Date(rawStatus.startTime).toISOString()
              : undefined,
          };
        }
        ...
        fastify.log.error(
          `Error getting enhanced server status for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send(
          createProblemDetails({
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        return {
          ...
          debug: {
            ...
            serverState: serverState || {
              message: "No tracked state for this server",
            },
        ...
        logger.error(
          `Error getting state debug for ${request.params.name}:`,
          error,
        );
        return reply.status(500).send({
          success: false,
