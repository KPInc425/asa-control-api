...
export default async function configRoutes(fastify) {
  ...
  // Game-Aware Config File Management
  // =================================
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        // Look up the server's game type from DB and validate against its adapter
  ...
  fastify.put(
    ...
    async (request, reply) => {
      try {
        ...
        // Validate fileName against the server's game adapter
        const serverRow = await configService.getServerInfo(serverName);
        const gameType = serverRow?.gameType || "ark";
        const adapter = gameFor(gameType);
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        try {
          const serverInfo =
            await configService.findServerConfigPath(serverName);
          if (serverInfo) {
            const serverPath =
            ...
            const serverConfigContent = await fs.readFile(
              serverConfigPath,
              "utf8",
            );
