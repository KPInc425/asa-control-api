...
# Build the service script content
...
function Start-Service {
    ...
    if (!(Test-Path `$envFile)) {
        `$envContent = "NODE_ENV=production`nPORT=`$Port`nSERVER_MODE=native`nNATIVE_BASE_PATH=G:\ARK`nJWT_SECRET=fallback-secret-change-in-production`nCORS_ORIGIN=http://localhost:4010`nLOG_LEVEL=info`nLOG_FILE_PATH=`$LogPath\app.log"
